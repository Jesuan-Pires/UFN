...
select distinct salario from funcionario where salario > 30000;
select * from funcionario nome where Pnome = "Joao";
...
#select * from funcionario where cpf_supervisor is not null order by Pnome  and Unome desc; ERRADO!!!
select * from funcionario where cpf_supervisor is not null order by Pnome,Unome desc;
select * from funcionario where salario order by SALARIO LIMIT 3 ; 
select min(salario) from funcionario where salario order by salario desc;
select * from funcionario where funcionario.salario = (select min(salario) from funcionario);
select * from funcionario where funcionario.salario = (select max(salario) from funcionario);
...
select concat(Pnome," ", Minicial, " ", Unome) as nome_completo from funcionario;
select * from funcionario;

select f.pnome, f.unome, d.dnumero, d.dnome from funcionario as f, departamento as d where D.dnumero = f.dnr and D.dnome = "Pesquisa";

select * from departamento, funcionario where Dnome = "administração" and Dnr = 4 and salario * 1.15;

select F.Pnome, F.Unome, F.salario, D.dnumero, F.salario * 1.15 as "aumento 15%"
from Funcionario as F inner join Departamento as D
where D.Dnumero = F.Dnr and D.Dnome = "Administração"
order by F.salario; #ou Like "%adm%"

select F.Pnome, F.Unome, F.endereco
from Funcionario as F inner join Departamento on Dnumero = Dnr
where Dnome = "pesquisa";

select * from funcionario inner join trabalha_em on Cpf = Fcpf inner join projeto on projnumero = pnr where Projnome = "produtox";
select * from funcionario, trabalha_em, projeto where Fcpf = cpf and Pnr = projnumero and projnome = "produtox";
select Projnumero, Dnumero, Unome, Endereco, Datanasc from funcionario inner join departamento on cpf = cpf_gerente inner join projeto on Dnumero = Dnum where Projlocal = "Mauá";
